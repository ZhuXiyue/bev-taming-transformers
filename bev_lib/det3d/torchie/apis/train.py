...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    if distributed:
        #model = apex.parallel.convert_syncbn_model(model) #torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)#apex.parallel.convert_syncbn_model(model)
    ...
    if distributed:
        ...
        device = torch.device('cuda:'+str(cfg.local_rank))
