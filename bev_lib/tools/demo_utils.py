...
class Box:
    ...
    def poly(self,
        ...
        res = []
        for i in range(4):
            res.append([corners.T[i][0],corners.T[i][1]])
...
#     color = np.array([(255,100,60),(255,60,60),(150,100,60),(255,0,100),(255,0,255),(255,0,0),(0,255,255)])
...
#     thrs = [0.5, 0.5, 0.7, 0.6, 0.4, 0.3, 0.9]
...
#     # ego car
#     array_to_print[256-10:256+10,256-5:256+5,:] = np.array([50,50,100])
#     return array_to_print.astype(int)

def visual_iou(points, gt_anno, det, i, eval_range=35, conf_th=0.5):
    axes_limit = 51.2 #eval_range + 3  # Slightly bigger to include boxes that extend beyond the range.
    gt_anno,gt_seg = gt_anno
    det,det_seg = det
    print('in vis',np.shape(gt_seg))
    _, ax = plt.subplots(1, 1, figsize=(9, 9), dpi=200)
    
    gt_seg_im = vis_layer(gt_seg)
    det_seg_im = vis_layer(det_seg)
    # print(np.max(det_seg_im),np.min(det_seg_im))
    ax.imshow(gt_seg_im,extent=[-axes_limit, axes_limit,-axes_limit, axes_limit])
